            ...
            bcrypt.compare(oldPassword, hash, function (error, result) {
                ...
                if (result === false) {
                    ...
                    const mailOptions = {
                        ...
                        `
                        // text: `Salut,\n\nAceasta este o confirmare că parola pentru contul dvs. tocmai a fost modificată.\n`
...
//forgot password reset 
 ...
 module.exports.postForgotPasswordReset = (req, res,next ) => {
    ...
    db.query('SELECT users.id, users.email,users.first_name,users.password, users.resetPasswordToken ,users.passwordResetExpires,users.type, users.user_status FROM  users WHERE resetPasswordToken = ? AND passwordResetExpires > NOW()', [req.params.token], function (err, rows, fields) {
        if (err) {
            console.log('[mysql error]', err)
        }
