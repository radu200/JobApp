          ...
          {reports.map((report, i) => (
            <TableRow key={report.id}>
              ...
              <TableCell>
                  ...
                  <Link to={`/api/company/${report.id}`} rel="noreferrer" target={"_blank"}> {report.first_name}</Link>
                  : report.type === 'jobseeker' ? 
                  <Link to={`/candidate-details/${report.id}` }  rel="noreferrer" target={"_blank"}> {report.first_name}</Link> : null}
             </TableCell>
