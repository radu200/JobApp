...
module.exports.postChangePassword = async (req, res, next) => {
  ...
  req
    .checkBody("oldPassword", "Password must be between 6-100 characters long.")
    .len(6, 100);
  req
    .checkBody("newPassword", "Password must be between 6-100 characters long.")
    .len(6, 100);
  req
  ...
  try {
    ...
    const [userDetails] = await db.execute(
      "SELECT users.id, users.email, users.password  FROM users WHERE id = ?",
      [req.user.id]
    );
    ...
    if (!match) {
      ...
      await Promise.all([
        ...
        send_emails.changePasswordProfile(
          req,
          res,
          next,
          nodemailer,
...
module.exports.postForgotPassword = async (req, res, next) => {
  req
    .checkBody("forgotPswEmail", " Te rog introdu o adresa de email valida.")
    .isEmail();
  ...
  try {
    ...
    if (userDetails.length === 0) {
      ...
      await Promise.all([
        db.execute(
          "UPDATE users SET forgotPasswordToken = ? , forgotPasswordTokenExpires = TIMESTAMPADD(HOUR, 1, NOW())  WHERE email = ?",
          [token, email]
        ),
...
module.exports.getForgotPasswordReset = async (req, res, next) => {
  try {
    const db = await dbPromise;
    const [userDetails] = await db.execute(
      "SELECT users.forgotPasswordtoken , users.forgotPasswordTokenExpires FROM  users WHERE forgotPasswordtoken = ? AND forgotPasswordTokenExpires > NOW()",
      [req.params.token]
    );
...
module.exports.postForgotPasswordReset = async (req, res, next) => {
  ...
  req
    ...
    .len(6, 100);
  req
    .checkBody("confirmNewPassword", "Passwords do not match")
    .equals(req.body.newPassword);
  ...
  try {
    ...
    const [userDetails] = await db.execute(
      "SELECT users.id, users.email,users.first_name,users.password, users.forgotPasswordtoken,users.forgotPasswordTokenExpires,users.type  FROM  users WHERE forgotPasswordToken = ? AND forgotPasswordTokenExpires > NOW()",
      [req.params.token]
    );
    ...
    await Promise.all([
      db.query(
        "UPDATE  users SET password = ? WHERE forgotPasswordToken = ? AND forgotPasswordTokenExpires > NOW()",
        [hashPassword, req.params.token]
      ),

      send_emails.forgotPasswordSucess(
        req,
        res,
        next,
        nodemailer,
...
module.exports.getCheckEmail = async (req, res, next) => {
  ...
  try {
    ...
    const [userDetails] = await db.execute(
      "SELECT id, password,type, email,first_name, last_name FROM users where email_confirmation_token = ? AND email_token_expire > NOW()",
      [token]
    );

    if (userDetails.length > 0) {
      await db.execute(
        "UPDATE users SET email_status = ? WHERE email_confirmation_token = ? AND email_token_expire > NOW()",
        ["verified", token]
      );

      await db.execute(
        "UPDATE users SET email_confirmation_token = ? WHERE id = ? ",
        [null, userDetails[0].id]
      );
      ...
      req.flash("error_msg", {
        msg:
          "Ne pare rau din pacate nu am putut sa va verificam emailul sau tokenul a expirat"
      });
...
module.exports.postResendEmailCheck = async (req, res, next) => {
  try {
    ...
    await Promise.all([
      db.execute(
        "UPDATE users SET email_token_expire = TIMESTAMPADD(HOUR, 1, NOW()),email_confirmation_token = ? WHERE  id = ? ",
        [token, req.user.id]
      ),

      send_emails.checkEmailAfterSignUp(
        req,
        res,
        nodemailer,
        userDetails[0].email,
...
module.exports.postChangeEmail = async (req, res, next) => {
  ...
  try {
    ...
    if (!match) {
      ...
      await Promise.all([
        conn.execute(
          "UPDATE users SET email = ?, email_status = ?,email_confirmation_token = ? , email_token_expire = TIMESTAMPADD(HOUR, 2, NOW()) WHERE id = ? ",
          [email, "unverified", token, req.user.id]
        ),
